    ...
    <div className={`fixed bottom-4 left-4 bg-black/90 text-white rounded-lg shadow-xl font-mono text-xs transition-all duration-200 ${
      ...
      {!isMinimized && (
        ...
        <div className="flex justify-between">
          ...
          <span className="text-blue-400">P{viewingPosition.round}P{viewingPosition.pick}</span>
        </div>
        
        <div className="flex justify-between">
          <span className="text-gray-400">Engine Status:</span>
          <span className={engineState.status === 'active' ? 'text-green-400' : 'text-gray-400'}>
            {engineState.status}
          </span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className={isViewingCurrent ? 'text-green-400' : 'text-red-400'}>
        ...
        <div className="flex justify-between">
          ...
          <span className="text-white">{engineState.actionHistory.length}</span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className="text-cyan-400 text-[10px]">
            {engineState.actionHistory.length > 0 
              ? engineState.actionHistory[engineState.actionHistory.length - 1].type
              : 'None'}
          </span>
        </div>
      </div>
